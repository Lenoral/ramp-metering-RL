...
class ReplayBuffer(object):
	...
	def load(self, save_folder, size=-1):
		reward_buffer = np.load("reward.npy")
		
		# Adjust crt_size if we're using a custom size
		size = min(int(size), self.max_size) if size > 0 else self.max_size
		self.size = min(reward_buffer.shape[0], size)

		self.state[:self.size] = np.load("state.npy")[:self.size]
		self.action[:self.size] = np.load("action.npy")[:self.size]
		self.next_state[:self.size] = np.load("next_state.npy")[:self.size]
		self.reward[:self.size] = reward_buffer[:self.size]
		self.not_done[:self.size] = np.load("not_done.npy")[:self.size]